...
xhr.onreadystatechange = function(){
    if(xhr.readyState == xhr.DONE){
        ...
        movies.forEach(function(movie){
            filtRows += tempRow(movie);
        });
        filtTable += tempTable({tRows: filtRows});
        ...
        //filtList += "</ul>"
        if(isFilter){
            document.getElementById("filter_list").innerHTML = filtTable;
            var filtList = '';
            filtList += tempList({items: movies});
            document.getElementById("name-list").innerHTML = filtList;
        }else{
            var dispTable = '';
            // var dispRow = '';
            // movies.forEach(function(movie){
            //     dispRow += tempRowCheckbox({movieStr: JSON.stringify(movie).replaceAll('"', "'")});
            //     dispRow += tempRemRow(movie);
            // });
            // dispTable += tempTableCb({tRows: dispRow});
            dispTable += tempCheckBoxTable({items: movies});
            document.getElementById("display_list").innerHTML = dispTable;
            // var ttemp = document.getElementById('tableTemplate').innerHTML;
            // var dispTable = '';
            // dispTable = _.template(ttemp);
            // console.log(dispTable({items: movies}));
            // document.getElementById("display_list").innerHTML = dispTable({items: movies});
            document.getElementById("filter_list").innerHTML = "";
            document.getElementById("name-list").innerHTML = "";
        }
    }
